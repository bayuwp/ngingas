...
const AdminDashboard = () => {
  ...
  const handleFileUpload = async (file, fieldName) => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/upload/materi`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        alert(err.error || 'Gagal mengunggah file');
        return null;
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (gambar) formData.append('foto', gambar);
      if (modul) formData.append('modul', modul);
      if (ppt) formData.append('ppt', ppt);
      if (tugas) formData.append('tugas', tugas);
  ...
  return (
    <div className="d-flex min-h-screen">
      <Sidebar />
      <div className="flex-grow p-6 bg-gray-50 ml-[50px]">
        <h1 className="text-3xl font-bold text-gray-800 mb-8">
          📚 Admin - Upload Materi Perkuliahan
        </h1>
        <div className="bg-white p-6 rounded-2xl shadow-md mb-10 max-w-full">
              ...
              {materiList.map((materi, index) => (
                <tr key={materi.id} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                  ...
                  <td className="px-4 py-3 border">
                    <a href={materi.linkVideo} target="_blank" rel="noopener noreferrer">
                      Tonton Video
                    </a>
                  </td>
                  <td className="px-4 py-3 border">
                    <a href={materi.namaModul} target="_blank" rel="noopener noreferrer">
                      Unduh Modul
                    </a>
                  </td>
                  <td className="px-4 py-3 border">
                    <a href={materi.namaPpt} target="_blank" rel="noopener noreferrer">
                      Unduh PPT
                    </a>
                  </td>
                  <td className="px-4 py-3 border">
                    <a href={materi.namaTugas} target="_blank" rel="noopener noreferrer">
                      Unduh Tugas
                    </a>
                  </td>
                  <td className="px-4 py-3 border">
